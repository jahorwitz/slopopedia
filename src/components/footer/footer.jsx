export const Footer = () => {
  return (
    <footer className="flex position: absolute bottom-0 left-0 right-0 mb-10 flex-row gap-10 justify-between max-w-[989px]  mx-auto xs:flex-col xs:ml-5 sm:ml-5  sm:flex-col">
      <div className="font-arial text-dark/60 text-sm xs:mb-4 sm:mb-4">
        <p className="">2023 The Slop Goblin's Web Reference</p>
      </div>
      <div className="font-arial text-dark/60 text-sm xs:mb-4 sm:mb-4">
        <p className="">Contact: hello@theslopgoblins.com</p>
      </div>
      <div className="font-arial text-dark/60 text-sm xs:mb-4 sm:mb-4">
        <p className="">Privacy policy</p>
      </div>
      <div className="font-arial text-dark/60 text-sm xs:mb-4 sm:mb-4">
        <p className="">Terms & Conditions</p>
      </div>
      <div className="font-arial text-dark/60 text-sm xs:mb-4 sm:mb-4">
        <p className="">Cookies</p>
      </div>
    </footer>
  );
};
