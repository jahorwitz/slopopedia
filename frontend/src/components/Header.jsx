...
Header.Logo = () => {
        return (
            ...
            <img className="w-80 pt-4 xs:w-60 md:w-70 " src={headerLogo} alt="slopopedia logo" />
                ...
                <div className="pt-5 right-0.5 absolute  xs:block sm:block  lg: hidden xl: hidden" >
                    <div className=" space-y-1  ">
                        {navLinks.map((link, index) => (
                            <a key={index} className="text-grey-300 bg-black hover:bg-gray-700 hover:text-white
                                block px-1 py-2 text-base font-medium border-b-2 gap-2.5"
                                href={link.link}>
                                <img className="w-5 h-5 inline-block mr-1 " src={link.src} alt={link.title} />
                                {link.title}
                            </a>
                        ))}
                    </div>
...
Header.Profile = () => {
    return (
        ...
        <div className="  flex flex-row h-[24px]    gap-2.5 md: gap-1 md:pl-4 md:flex  lg:flex xl:flex sm: hidden xs: hidden ">
            <a href="#" className=" border-b-2  ">Log in</a>
            <p>/</p>
            <a href="#" className=" border-b-2 ">Sign Up</a>
